...
<head>
    ...
    <noscript><img height="1" width="1" style="display:none"
            src="https://www.facebook.com/tr?id=943948765000391&ev=AddPaymentInfo&noscript=1" /></noscript>
    <!-- End Meta Pixel Code -->
</head>
...
<body class="bg-surface text-on-surface min-h-screen flex flex-col">
    ...
    <nav class="fixed top-0 w-full z-50 bg-white/90 backdrop-blur-md border-b border-surface-variant/20 shadow-sm">
        <div class="flex justify-between items-center max-w-7xl mx-auto px-6 h-16">
            <a href="https://ekspor.wamapss.com"
                class="text-2xl font-black tracking-tighter text-primary flex items-center gap-2">
            ...
            <div class="text-center mb-8">
                ...
                <p class="text-on-surface-variant text-sm md:text-base font-medium max-w-xl mx-auto leading-relaxed">
                    @if($transaction->method === 'QRIS')
                        Silakan pindai kode QRIS di bawah ini melalui aplikasi e-wallet atau M-Banking Anda untuk aktivasi
                        instan.
                    @else
                        Silakan selesaikan pembayaran ke nomor Virtual Account atau nomor pembayaran di bawah ini untuk
                        aktivasi instan.
                    @endif
                </p>
            </div>
                    ...
                    <div
                        ...
                        @else
                            <div class="py-6 md:py-10 flex flex-col items-center">
                                <div
                                    class="w-16 h-16 md:w-20 md:h-20 bg-primary/10 rounded-full flex items-center justify-center mb-6">
                                    <span
                                        class="material-symbols-outlined text-4xl md:text-5xl text-primary">account_balance</span>
                                </div>
                                <p class="text-[10px] uppercase font-black text-primary tracking-[0.3em] mb-6 text-center">
                                    {{ $transaction->method }} TRANSFER
                                </p>

                                <div class="w-full max-w-md px-4">
                                    <div
                                        class="bg-surface-container-low p-5 md:p-8 rounded-[2rem] border-2 border-dashed border-primary/30 text-center">
                                        <p
                                            class="text-[10px] md:text-xs font-bold text-on-surface-variant uppercase mb-4 tracking-widest">
                                            Nomor Virtual Account</p>
                                        <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
                                            <span id="va-number"
                                                class="text-2xl sm:text-3xl md:text-4xl font-black text-primary tracking-wider break-all leading-tight">{{ $transaction->payment_number ?? '--------' }}</span>
                                            <button onclick="copyToClipboard('va-number')"
                                                class="flex items-center justify-center p-3 bg-white rounded-2xl shadow-sm hover:shadow-md active:scale-95 transition-all text-primary border border-primary/10 group flex-shrink-0">
                                                <span
                                                    class="material-symbols-outlined text-xl group-hover:rotate-12 transition-transform">content_copy</span>
                                            </button>
                                        </div>
                                    </div>
                                </div>
                                <div class="mt-8 bg-blue-50/50 p-4 rounded-2xl border border-blue-100/50 max-w-sm">
                                    <p class="text-[11px] font-bold text-on-surface-variant text-center leading-relaxed">
                                        Gunakan nomor Virtual Account di atas untuk melakukan transfer melalui <br
                                            class="hidden sm:block"><strong>Mobile Banking, ATM, atau Internet
                                            Banking</strong> Anda.
                                    </p>
                                </div>
                            </div>
